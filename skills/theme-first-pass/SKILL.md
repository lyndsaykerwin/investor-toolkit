...
1. **Criteria.** If `shared/Firm_Profile.md` still shows the marker `<!-- TEMPLATE — replace via sourcing-setup -->`, run the `sourcing-setup` skill first — it's a **short structured intake** (investor type, target size, funding ceiling, in/out industries, plus an optional catch-all), then it hands straight back here. Finer product-feature filters are captured later, when `enrich-companies` first needs them. Most users land here first, so expect that intake on the first run.
...
- **6 — Firm thesis fit.** Fill when Sections 3-5 have content and a real wedge surfaces. Check the investor type in `shared/Firm_Profile.md`: for a **strategic acquirer**, synergy with the named portfolio/platforms is a required lens here; for a **financial buyer** it's optional, included only if relevant. Otherwise leave `> No clear thesis surfaced — revisit after enrichment`. After drafting this section, **ask the user which thesis areas resonate**, and record their steer here as operator-confirmed — that steer becomes the lens `market-landscape` and `enrich-companies` use to prioritize names and answer specific questions about companies.
