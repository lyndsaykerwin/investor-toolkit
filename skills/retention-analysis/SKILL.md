...
Turn a workbook of customer-level revenue into a three-sheet deliverable: **Corkscrew** (rollforward + retention metrics), **Helper** (Raw Data with Analysis), and **Raw Data** (verbatim copy). Every metric is a live formula referencing the helper, which references the raw data. Change one customer's revenue in Raw Data and the entire model flexes.
...
- **Revenue-type scope** (the user-confirmed in-scope revenue types, plus any source labels that are equivalent recurring-revenue tags such as subscription, license, platform, MRR, ARR, monthly, or annual)
...
- **If the raw data can be summed directly** (single in-scope revenue type, no per-customer aggregation needed) — write the check formula against the declared Raw Data interface, e.g. `=<Ending_cell> - SUMIFS(RawData_2025_01, RawData_Type, <included_type>) * $ARR_factor`. No helper-sheet self-validation block needed.
...
- **Decomposed display by revenue type** — when multiple revenue types are in scope, surface one row per included source type × ARR factor above ONE variance row at the bottom (Variance = Ending − Sum of components). The rows are generated dynamically; do not assume one, two, or three type names. Out-of-scope types stay visible on the helper sheet with an exclusion reason.

### 4a. Declare the calculation interface before writing formulas

Before writing the Corkscrew, generate stable names for the actual ranges being modeled:

- `RawData_Customers`, `RawData_Type` when a type column exists, and `RawData_<YYYY_MM>` for each source month
- `Cube_Customers`, `Cube_<YYYY_MM>` for each helper customer revenue range
- `CubeType_<normalized_type>_<source_type>_<YYYY_MM>` for each revenue-type summary cell

Use those generated names in formulas. Do not embed fixed data-row ranges such as `G12:G108` in Corkscrew formulas. The only place row numbers should exist is inside the generated names or as local row labels in the visible workbook layout.

Parse source months to internal keys like `2022-01`; keep visible labels exactly as the source presents them or in the workbook's chosen display style. Parse revenue-type labels to normalized IDs such as `recurring`, `non_recurring`, or a sanitized custom ID, while preserving the source label on screen.

### 5. Investigate failures before surfacing

If a reconciliation row is non-zero after recalc, do not just hand the file to the user with a red flag. Walk the diagnosis yourself: is it a named-range off-by-one, a type-scope mismatch, an equivalent revenue tag that was misclassified, or a customer range that excludes source rows? Fix it, re-run, then deliver. Only escalate genuine blockers (e.g., the source data itself doesn't tie to its own stated totals).
...
- **Counts:** `COUNTIF(Cube_2025_01, ">0")`, not hardcoded row ranges
- **Conditional sums:** `SUMIFS(...)`, not `SUMPRODUCT((cond)*values)`
- **Declared range lookup:** `SUMIFS(RawData_2025_01, RawData_Type, <included_type>)` or `SUMIFS(RawData_2025_01, RawData_Customers, $A<row>, RawData_Type, <included_type>)` — the month key is generated once, then formulas consume the named range
- **Reference repeated metrics, don't recompute.** Compute once on the helper sheet, pull into the Corkscrew through generated names or a direct cell reference
- **Keep SUMPRODUCT only where it earns its keep:** differential row-level math across two declared ranges (e.g., customers active in both period *t* and period *t-lookback*). That's the one case SUMIFS can't handle.
...
- **Aggregating (`Raw Data with Analysis`)** — source has multiple rows per customer (one per product line). Each helper cell aggregates with `SUMIFS`. For wide-block source data, the customer-row formula uses **direct column reference** for speed (thousands of customer rows × dozens of columns); the summary-block formulas at the top of the helper use `SUMIFS(INDEX(...), MATCH(...))` so they copy horizontally without hand-editing.
...
2. **Re-open with `openpyxl.load_workbook(path, data_only=True)`** and verify:
   ...
   - No check or metric formula uses `IFERROR(..., 0)` to hide a calculation problem. Use `IF(denominator=0,"",formula)` for legitimately blank retention periods.

   If LibreOffice is unavailable in the current environment, fail the release gate when `--require-libreoffice` is set. For local desktop validation, also run the bundled formula calculation fallback and scan the calculated check rows; do not claim a LibreOffice recalc occurred unless `soffice` actually ran.
...
Three sheets total, in this order from leftmost: **Corkscrew**, **Raw Data with Analysis**, **Raw Data**.
...
Row 17    # Active (prior period)    [COUNTIF over Cube_<prior_key>]
Row 18    # Active (current)         [COUNTIF over Cube_<curr_key>]
...
Row 32+   One generated row per included revenue type
          [= CubeType_<normalized_type>_<source_type>_<YYYY_MM> × $ARR_factor]
Next row  Sum in-scope ARR
Next row  Variance vs Ending ARR     [= Sum in-scope ARR − Ending ARR]   must = 0
```
...
```
Row 1   Month headers           visible labels for the internal month keys. Col A label = "Customer ID"
Row 2   # Active customers      = COUNTIF(Cube_<YYYY_MM>, ">0")
Row 3   # Retained vs prior     For first N columns (N = lookback) blank — no prior period yet.
                                From col N+1 onward:
                                = SUMPRODUCT((Cube_<curr_YYYY_MM>>0) *
                                             (Cube_<prior_YYYY_MM>>0))
Row 4   Check # Active vs Raw   Independent recount directly from Raw Data, must = 0
Rows 6+ Revenue type totals     One generated row for every source revenue type, included or excluded:
                                = SUMIFS(RawData_<YYYY_MM>, RawData_Type, "<source type label>")
                                Metadata columns to the right show Included/Excluded,
                                normalized key, and reason.
Next row Total MRR (all visible types)
Next row In-scope MRR           Sum only user-confirmed included types.
Next row Check vs Raw Data      = Total MRR − SUM(RawData_<YYYY_MM>)   must = 0
Next row Check in-scope vs customers
                                = In-scope MRR − SUM(Cube_<YYYY_MM>)   must = 0
Customer data                   Col A = Customer ID. Each month cell uses the declared interface:
                                = SUMIFS(RawData_<YYYY_MM>, RawData_Customers, $A<row>,
                                         RawData_Type, "<included source type label>")
```

**Formula style summary:**
- Summary block: generated revenue-type rows, one per discovered source label
- Customer rows: `SUMIFS` over generated Raw Data names
- Corkscrew rows: `SUMPRODUCT` over generated Cube names
- Check rows: external Raw Data tie-out plus in-scope customer-row tie-out
...
All Corkscrew formulas reference the helper sheet through generated names. For each Corkscrew comparison-period column, resolve the **current period** and **prior period** by internal month key, not by column letter or row number.

**Column mapping — work this out before writing any formulas.** For any lookback `N` over `M` source months:

| Corkscrew offset | Period key | Cube current range | Cube prior range |
|---|---|---|---|
| 0 | `months[N]` | `Cube_<months[N]>` | `Cube_<months[0]>` |
| 1 | `months[N+1]` | `Cube_<months[N+1]>` | `Cube_<months[1]>` |
| last | `months[M-1]` | `Cube_<months[M-1]>` | `Cube_<months[M-N-1]>` |

Rule: for Corkscrew column at offset `i` from the first comparison-period column, current key is `months[lookback + i]`, prior key is `months[i]`. The formulas consume generated names such as `Cube_2025_01`.

**Rollforward formulas** (`<curr_key>` and `<prior_key>` are internal month keys):

```
Beginning ARR  =SUMPRODUCT((Cube_<prior_key>>0)*Cube_<prior_key>)*$ARR_factor

New ARR        =SUMPRODUCT((Cube_<prior_key>=0)*
                          (Cube_<curr_key>>0)*
                          Cube_<curr_key>)*$ARR_factor

Upsell         =SUMPRODUCT((Cube_<prior_key>>0)*
                          (Cube_<curr_key>>Cube_<prior_key>)*
                          (Cube_<curr_key>-Cube_<prior_key>))*$ARR_factor

Downsell       =SUMPRODUCT((Cube_<prior_key>>0)*
                          (Cube_<curr_key>>0)*
                          (Cube_<curr_key><Cube_<prior_key>)*
                          (Cube_<curr_key>-Cube_<prior_key>))*$ARR_factor

Churn          =SUMPRODUCT((Cube_<prior_key>>0)*
                          (Cube_<curr_key>=0)*
                          (-Cube_<prior_key>))*$ARR_factor

Ending         =<col>8+<col>9+<col>10+<col>11+<col>12

External Check =<Ending cell> - (SUM(Cube_<curr_key>) × $ARR_factor)
               // the helper's own check ties Cube_<curr_key> back to RawData_<curr_key>
```

**Customer-count formulas:**

```
# Active prior    =COUNTIF(Cube_<prior_key>, ">0")
# Active current  =COUNTIF(Cube_<curr_key>, ">0")
# Churned         =SUMPRODUCT((Cube_<prior_key>>0)*(Cube_<curr_key>=0))
# New             =SUMPRODUCT((Cube_<prior_key>=0)*(Cube_<curr_key>>0))
```

**Retention metrics** (do not use `IFERROR(...,0)`):

```
GRR    =IF(<Beginning>=0,"",(<Beginning> + <Downsell> + <Churn>) / <Beginning>)
NRR    =IF(<Beginning>=0,"",(<Beginning> + <Upsell> + <Downsell> + <Churn>) / <Beginning>)
Logo   =IF(<Active prior>=0,"",(<Active prior> - <Churned>) / <Active prior>)
```
...
2. **Cite specific cells in both Raw Data and the helper.** Example: `"Customer 45 lives in Raw Data with Analysis row 55. N55 (Jan-22 in-scope MRR) = 2,669; Z55 (Jan-23) = 10,659. The source revenue type labeled 'Subscription Platform' was normalized as recurring and drives the expansion from Raw Data row 244 (InsightDash)."` The user must be able to click directly to the cell.
...
|---|---|---|---:|---:|
| 241 | MergeLogic | Implementation fee → **excluded** | ~~$0~~ | ~~$0~~ |
| 243 | FlowBuilder | Annual subscription | $764 | $764 |
| 244 | InsightDash | Platform license | $1,905 | $9,895 |
...
- [ ] Three sheets in the deliverable: Corkscrew + Raw Data with Analysis + Raw Data
